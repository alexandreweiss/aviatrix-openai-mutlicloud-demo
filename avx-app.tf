resource "azurerm_resource_group" "app_rg" {
  location = var.azure_r1_location
  name     = "oai-${var.azure_r1_location_short}-app-rg"
}


resource "azurerm_storage_account" "app_aci_sa" {
  name                     = "acisa${random_integer.random.result}"
  resource_group_name      = azurerm_resource_group.app_rg.name
  location                 = var.azure_r1_location
  account_tier             = "Standard"
  account_replication_type = "LRS"
  account_kind             = "StorageV2"
}

resource "azurerm_storage_share" "app1_aci_share" {
  name               = "aci-config"
  storage_account_id = azurerm_storage_account.app_aci_sa.id
  quota              = 1
}

resource "local_file" "app1_config_yaml" {
  filename = "app1-config.yaml"
  content = templatefile("${path.module}/gatus_config.tpl",
    { "customer_name"    = var.customer_name,
      "application_1"    = var.application_1,
      "customer_website" = var.customer_website
    }
  )
}


resource "azurerm_storage_share_file" "app1_config_file" {
  name             = "config.yaml"
  content_type     = "text/yaml"
  source           = local_file.app1_config_yaml.filename
  storage_share_id = azurerm_storage_share.app1_aci_share.url
  lifecycle {
    replace_triggered_by = [null_resource.app1_always_run]
  }
}

resource "null_resource" "app1_always_run" {
  triggers = {
    timestamp = "${timestamp()}"
  }
}

resource "azurerm_container_group" "app_container_group" {
  name                = "${var.application_1}-cg"
  resource_group_name = azurerm_resource_group.app_rg.name
  location            = var.azure_r1_location
  depends_on          = [module.azr_r1_spoke_oai, azurerm_storage_share_file.app1_config_file]

  container {
    name   = "gatus"
    image  = "aviatrixacr.azurecr.io/aviatrix/gatus-aviatrix:latest"
    cpu    = "1"
    memory = "1.5"
    ports {
      port     = 8080
      protocol = "TCP"
    }
    volume {
      name                 = "config"
      share_name           = "aci-config"
      mount_path           = "/config"
      storage_account_key  = azurerm_storage_account.app_aci_sa.primary_access_key
      storage_account_name = azurerm_storage_account.app_aci_sa.name
    }
  }
  exposed_port = [{
    port     = 8080
    protocol = "TCP"
  }]
  ip_address_type = "Private"
  subnet_ids      = [azurerm_subnet.r1-azure-spoke-oai-aci-subnet.id]
  os_type         = "Linux"
}

resource "aviatrix_smart_group" "azr-MyApp1-sg" {
  name = "${var.application_1}-app-vnet"
  selector {
    match_expressions {
      type = "vpc"
      name = azurerm_virtual_network.azure-spoke-oai-r1.name
    }
  }
}

resource "aviatrix_web_group" "allowed_domains" {
  name = "${var.application_1}-allowed-domains"
  selector {
    match_expressions {
      snifilter = var.customer_website
    }
  }
}
